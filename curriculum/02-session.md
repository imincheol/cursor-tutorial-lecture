...
### Debug Mode (자동 로그 삽입)
Copilot은 코드만 보고 추측하지만, Cursor는 실제로 로그를 삽입해서 실행하며 버그를 추적합니다.

**예시**:
- "로그인 버그 고쳐줘" → 자동으로 `console.log` 삽입
- 의심 지점마다 로그 추가 → 실행 → 원인 발견
- 버그 수정 후 로그 자동 제거

### Visual Editor (브라우저 UI 편집)
브라우저에서 마우스로 UI 요소를 직접 선택하고 Agent에게 수정 요청할 수 있습니다.
