...
- [2부: Clarification Questions와 Subagents](#2부-clarification-questions와-subagents)
- [3부: 실전 활용](#3부-실전-활용)
...
## 2부: Clarification Questions와 Subagents

### Clarification Questions 복습

Clarification Questions는 Plan 모드의 차별화 기능입니다 (자세한 내용은 [2장 참조](../session-02-differentiators/README.md#1부-clarification-questions)).

**핵심 요약**:
- Agent 모드: 텍스트 질문 → 타이핑 필요
- Plan 모드: UI 화면 → 클릭으로 선택
- 요구사항을 명확히 하여 재작업 감소

### Subagents와 함께 사용하기

복잡한 작업을 Subagents로 분배하기 전에 **Clarification Questions로 요구사항을 명확히** 하면:

1. **각 Subagent에 정확한 지시 전달**
   - 불명확한 요구사항 → 각 Subagent가 다른 방향으로 구현
   - 명확한 요구사항 → 모든 Subagent가 일관되게 구현

2. **재작업 최소화**
   - 처음부터 올바른 방향으로 분배
   - Subagent 결과를 다시 수정할 필요 없음
...
9장에서는 **Multi-Agent**를 배웁니다. Subagents가 자동 분배라면, Multi-Agent는 개발자가 직접 여러 Agent를 병렬로 실행하는 수동 방식입니다. Git Worktree와 함께 사용하면 강력한 병렬 개발이 가능합니다.
