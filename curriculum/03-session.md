...
### 대화형 CLI Agent (지금 이 대화처럼!)
터미널에서 지금 우리가 대화하는 것처럼 Agent와 대화하며 코드 작업을 합니다.

**예시**:
```bash
cursor agent

> 이 프로젝트에서 가장 큰 파일 찾아줘
> 첫 번째 파일 내용 요약해줘
> 리팩토링 필요한 부분 있어?
> 첫 번째부터 적용해줘
```
...
**특징**:
- 단계별로 대화하며 진행 (한 번에 완벽한 질문 안 해도 됨)
- 컨텍스트 유지 ("그것", "이전 파일" 표현 가능)
- 파일 직접 수정 가능

### Shell Mode (명령어 생성 전용)
자연어를 터미널 명령어로 변환해줍니다. **코드 수정은 안 하고 명령어만 생성**합니다.
