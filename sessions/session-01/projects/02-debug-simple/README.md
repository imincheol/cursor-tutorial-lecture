...
위의 `calculator.js` 파일을 이 폴더에 생성하세요.

---

### Step 2: 일반 모드로 버그 수정 시도 (3분)

**Agent 모드**를 열고 다음을 요청하세요:

```
calculator.js의 버그를 찾아서 수정해줘
```

**예상 결과**:
- AI가 코드를 보고 **추측**으로 수정할 것입니다
- "아마도 이 부분이 문제일 것 같습니다..."
- 0으로 나누는 경우를 처리하지 못할 수 있습니다
- modulo 연산을 추가하지 못할 수 있습니다

---

### Step 3: Debug 모드로 버그 수정 (4분)
...
```
calculator.js를 실행해서 어떤 입력에서 문제가 발생하는지 확인하고 수정해줘
```
